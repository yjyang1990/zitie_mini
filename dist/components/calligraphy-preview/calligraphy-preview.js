...
Component({
    ...
    lifetimes: {
        attached() {
            ...
            // 设置默认字体样式
            const defaultFont = this.data.fontOptions.find((font) => font.id === this.properties.defaultFont);
            if (defaultFont) {
                this.setData({
    ...
    methods: {
        ...
        onFontChange(event) {
            ...
            const selectedFont = this.data.fontOptions.find((font) => font.id === fontId);
            if (selectedFont) {
                this.setData({
                    selectedFont: fontId,
